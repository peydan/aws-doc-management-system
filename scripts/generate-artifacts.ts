...
const FRONTEND_REACT_TS_OUTPUT_PATH = path.resolve(__dirname, '../frontend-react/src/generated/templates.ts');
const REACT_PUBLIC_DIR = path.resolve(__dirname, '../frontend-react/public');
...
`;
  ...
  if (fs.existsSync(path.dirname(FRONTEND_REACT_TS_OUTPUT_PATH))) {
    const tsContent = `/**
 * Auto-generated by scripts/generate-artifacts.ts
 * DO NOT EDIT MANUALLY. To update defaults, update the corresponding JSON Schema in schemas/
 */
export const SHARED_BASE_TEMPLATE = ${JSON.stringify(sharedDefaults, null, 2)} as const;

export const CLASS_SPECIFIC_TEMPLATES = ${JSON.stringify(classSpecificTemplates, null, 2)} as const;

export const METADATA_TEMPLATES = ${JSON.stringify(metadataTemplates, null, 2)} as const;
`;
    fs.writeFileSync(FRONTEND_REACT_TS_OUTPUT_PATH, tsContent, 'utf-8');
    console.log(`Generated React templates: ${FRONTEND_REACT_TS_OUTPUT_PATH}`);
  }
}
...
function syncFrontendDist() {
  ...
  // Also sync config.json to frontend-react/public
  const configSrc = path.join(frontendDir, 'config.json');
  if (fs.existsSync(configSrc) && fs.existsSync(REACT_PUBLIC_DIR)) {
    fs.copyFileSync(configSrc, path.join(REACT_PUBLIC_DIR, 'config.json'));
    console.log(`Synced config.json -> frontend-react/public/config.json`);
  }
}
