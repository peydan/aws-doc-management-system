...
| **Amazon Bedrock & AgentCore** | Managed agent execution loop (AgentCore Harness) with Amazon Nova 2 Lite (`us.amazon.nova-2-lite-v1:0`, 1M token window) and Model Context Protocol (MCP) tool routing via AgentCore Gateway. | Provides conversational reasoning, grounded document Q&A, and tool dispatch without custom orchestration frameworks; native support for real-time SSE streaming and ephemeral session memory. |
