...
12. **Cost Analysis & Sizing Model Synchronization**: Any architectural modification introducing new AWS services, compute runtimes, LLM token usages (e.g. Bedrock models), transient caches/derivatives, export archives, or storage lifecycles must be evaluated in Layer 6 impact analysis. Agents must update the pricing catalog, transaction formulas, and multi-tier estimates in `COST_ANALYSIS_AND_ESTIMATION_ISRAEL_REGION.md` and keep the interactive `presentations/cost_calculator.html` in sync, syntax-valid, and mathematically sound.
13. **API Gateway CORS Preflight & Binary Media Types Invariant**: When `binaryMediaTypes: ['*/*']` is enabled on `RestApi`, all HTTP `OPTIONS` preflight methods must use pure `MockIntegration` (never attach a Lambda function across all routes, which exceeds AWS Lambda's 20,480-byte resource policy quota). Both the Mock Integration request and its 200 Integration Response must explicitly specify `contentHandling: apigateway.ContentHandling.CONVERT_TO_TEXT` and `passthroughBehavior: apigateway.PassthroughBehavior.WHEN_NO_MATCH` with template `{"statusCode": 200}` to prevent API Gateway from failing with `500 Unable to transform request` on empty browser preflight requests. All changes must be verified using `test/unit/api-stack.test.ts`.
14. **Diagrams & Presentations Synchronization in Impact Analysis**: Updating architectural diagrams, sequence flows, and presentations is an integral, mandatory invariant of Layer 6 impact analysis. Whenever an architectural mutation, new endpoint, new AWS service (e.g. Amazon Bedrock), modified data flow, pipeline addition, or UI change is made, agents must evaluate and synchronize: (1) High-Level Architecture (`presentations/diagrams/aws_document_management_architecture.drawio` & exported `.png`), (2) UI & Backend Integration Architecture (`presentations/diagrams/ui_backend_integration_architecture.drawio`, `.png`, and `.svg`), (3) API Sequence Diagrams (`presentations/diagrams/api_sequence_diagrams/*.drawio`, corresponding `.jpg` exports, and the master workbook `all_api_sequence_diagrams.drawio`), (4) Mermaid sequence diagrams and CDK stack dependency graphs in `SOLUTION_ARCHITECTURE_SPECIFICATION.md`, and (5) presentation generators and decks (`presentations/generate_deck.py`, `presentations/generate_deck_hebrew.py`, and `presentations/*.html`). All modified or new `.drawio` files must be re-exported to raster images using the draw.io CLI.
