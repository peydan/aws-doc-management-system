...
| **14** | **Batch Multi-Document ZIP Export** | REST API | `POST /documents/batch-download` | `Document.Reader` | Bundles multiple documents into a ZIP archive with audit `manifest.json` and optional metadata; serves via 15-minute presigned URL or direct stream; 3-day S3 lifecycle expiration. |
...
| **17** | **Multi-Attribute & Full-Text Search** | REST API | `POST /search` | `Document.Reader` | Executes high-performance search across OpenSearch Serverless with field filtering, ranges, pagination, and text matching. |
...
| **19** | **Automated Fault Tolerance & DLQ** | Background Pipeline | SQS Dead Letter Queues | System / CloudWatch | Isolates failing events across all pipelines (Index DLQ, Stream DLQ, Enrichment DLQ) after 3 retries; triggers CloudWatch Alarms for immediate operator intervention. |
...
| **29** | **AI-Assisted Metadata Pre-Fill for UI** | REST API & Frontend UI | `POST /metadata/suggest` | `Document.Reader` | Stateless pre-upload AI metadata extraction from document excerpt or file bytes via Amazon Bedrock (Amazon Nova 2 Lite - `us.amazon.nova-2-lite-v1:0`); dynamically classifies document class and auto-switches UI document class dropdown; partitions extracted attributes into shared banking and class-specific traits; auto-populates web console inputs prior to immutable persistence. |
...
- **Dead Letter Queue (DLQ) Isolation & Alerting**: Failing events across all async pipelines (OpenSearch indexing, DynamoDB Stream CDC processing, and Amazon Bedrock LLM metadata enrichment) are isolated into dedicated SQS DLQs after 3 failed attempts, preventing poison-pill stalls. Each DLQ is wired directly to CloudWatch alarms triggering SNS alerts on >= 1 message.
